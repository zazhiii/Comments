...
`redisson` 实现了分布式锁，解决了上述问题
1. 添加依赖
2. 配置 Redisson

`Redisson` 可重入锁的原理：

可重入：同一个线程可以多次获得同一个锁，而不会发生死锁。

关键在额外维护了一个计数器，每次加锁时计数器加一，解锁时计数器减一，直到计数器为 0 时释放锁。
...
## `Redis` 秒杀优化
1. 新增优惠券的同时，把库存信息保存到 Redis 中
2. 用 Lua 脚本判断库存是否充足 和 是否重复下单
3. 若有资格则将抢购任务加入阻塞队列
4. 开启线程任务，从阻塞队列中取出任务，执行异步下单操作

基于阻塞队列的异步秒杀存在的问题：
1. 内存限制问题：若队列满了，后来的任务会被丢弃
2. 数据安全问题：若任务执行失败，会导致数据不一致。

## `Redis` 实现消息队列 优化秒杀
1. 使用 Redis 的 List 数据结构作为消息队列
2. 使用 Redis 的 `PubSub` 作为消息队列
3. 使用 Redis 的 `Stream` 作为消息队列

# 探店(笔记)

## 点赞
1. 同一个用户只能点赞一次，点赞后可以取消点赞

> 1. 可以维护一张数据库表，用户和文章的点赞关系表，但是这样会增加数据库的压力
> 
> 2. 用 Redis 的 Set 数据结构维护文章的点赞用户集合，没点过赞则添加，点过赞则删除

## 点赞排行榜

1. 文章查询前五个点赞的人
> 1. 用 Redis 的 ZSet 数据结构维护文章的点赞用户集合，分数为点赞时间戳

# 关注

## 关注和取关
维护了一张关注关系表，表中存储了用户和用户之间的关注关系
1. 关注和取关：插入和删除关注关系即可
2. 查询关注列表：查询关注关系表即可

## 共同关注
1. 将每个用户的关注列表维护为一个 Redis 中的 Set
2. 通过 Redis 的交集操作求出共同关注的用户
